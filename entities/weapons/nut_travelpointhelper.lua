-- Originally created by BlackTea: https://github.com/rebel1324/NutScript/blob/1.1-beta/plugins/area/entities/weapons/nut_areahelper.lua
AddCSLuaFile()

if (CLIENT) then
	SWEP.PrintName = "TravelPoint Helper"
	SWEP.Slot = 0
	SWEP.SlotPos = 0
	SWEP.CLMode = 0
end
...
if (SERVER) then
	...
	function SWEP:Reload()
		if areaPoint.startVector and areaPoint.endVector then
			print("minVector: " .. tostring(areaPoint.startVector))
			print("maxVector: " .. tostring(areaPoint.endVector))
			areaPoint = {}
	...
	function SWEP:DrawHUD()
		local w, h = ScrW(), ScrH()
		local cury = h/4*3
		local tx, ty = draw.SimpleText("Left Click: Set a vector with TracePos", "nutMediumFont", w/2, cury, color_white, 1, 1)
		cury = cury + ty
		local tx, ty = draw.SimpleText("Right Click: Set a vector with GetPos", "nutMediumFont", w/2, cury, color_white, 1, 1)
		cury = cury + ty
		local tx, ty = draw.SimpleText("Reload: Finish and output minVector and maxVector to console", "nutMediumFont", w/2, cury, color_white, 1, 1)
